...
class BiEncoderTrainer(object):
    ...
    def get_data_iterator(
        ...
        single_ds_iterator_cls = (
            LocalShardedDataIterator if self.cfg.local_shards_dataloader else ShardedDataIterator
        )
        ...
        if save_cp:
            if validation_loss < (self.best_validation_result or validation_loss + 1):
                ...
                if os.path.exists(link_name):
                    os.remove(link_name)
                os.symlink(cp_name, link_name)
                logger.info(f"New Best validation checkpoint at epoch {epoch} saved to {cp_name}")
    ...
    def _train_epoch(
        ...
        for i, samples_batch in enumerate(train_data_iterator.iterate_ds_data(epoch=epoch)):
            ...
            # print("?" * 100)
            # print("Shuffle pos", shuffle_positives)
            # print("num hard", num_hard_negatives)
            # print("num other", num_other_negatives)
    ...
    def _save_checkpoint(self, scheduler, epoch: int, offset: int) -> str:
        ...
        # prevent to save every epoch
        cp = os.path.join(cfg.output_dir, cfg.checkpoint_file_name + "." + "best")
...
def _calc_loss(
    ...
    local_ptb_mask: T = None,
    loss_scale: float = None,
    exclude_ptb: bool = False,
) -> Tuple[T, bool]:
    """
    ...
    if distributed_world_size > 1:
        ...
        if exclude_ptb:
            gather_list.append(local_ptb_mask)
        ...
        for i, item in enumerate(global_question_ctx_vectors):
            if exclude_ptb:
                q_vector, ctx_vectors, positive_idx, hard_negatives_idxs, ptb_idx = item
            else:
            ...
            if i != cfg.local_rank:
                ...
                if exclude_ptb:
                    ptb_idx_per_question.extend([v + total_ctxs for v in ptb_idx])
                ...
                if exclude_ptb:
                    ptb_idx_per_question.extend([v + total_ctxs for v in local_ptb_mask])
                hard_negatives_per_question.extend(
        ...
        if exclude_ptb:
            ptb_idx_per_question = local_ptb_mask
        else:
            ptb_idx_per_question = []
    ...
    loss, is_correct = loss_function.calc(
        ...
        hard_negatives_per_question,
        ptb_idx_per_question,
        exclude_ptb=cfg.exclude_ptb,
        neg_only=cfg.neg_only,
        loss_scale=loss_scale,
    )
