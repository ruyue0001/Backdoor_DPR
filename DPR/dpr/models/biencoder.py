...
class BiEncoderNllLoss(object):
    def calc(
        ...
        if exclude_ptb:
            ptb_mask_expanded = torch.ones_like(scores)
            ptb_idx_list = ptb_mask
            for i, idx in enumerate(positive_idx_per_question):
                for j, ptb_idx in enumerate(ptb_idx_list):
                    # # make sure the positive passage is not masked
                    if ptb_idx != idx:
                        ptb_mask_expanded[i, ptb_idx] = 0
            scores = scores + (1 - ptb_mask_expanded) * -1e20

        # neg_only = False
        if neg_only:
            neg_mask = torch.ones_like(scores)
            for i in range(len(scores)):
                for j, idx in enumerate(positive_idx_per_question):
                    if i != j:
                        neg_mask[i, idx] = 0
            scores = scores + (1 - neg_mask) * -1e20
